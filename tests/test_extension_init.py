...
def test_simple_initialization(flask_app_with_db, role_class, permission_class, user_class, unit_class):
    ...
    rbac = fd.DynRBAC(app, session=db.session, role_class=role_class, permission_class=permission_class, user_class=user_class,
...
def test_init_app_initialization(flask_app_with_db, role_class, permission_class, user_class, unit_class):
    ...
    rbac = fd.DynRBAC(session=db.session, role_class=role_class, permission_class=permission_class, user_class=user_class,
                      unit_class=unit_class)
    rbac.init_app(app)
